...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        True
        """
        # 这个地方我修改了顺序，包括后续三种牌，我都把这个条件前置了。。。
        # 但是我不知道为啥官网上的顺序是要放在中间。。。
        added = False
        # You should add your implementation above this.
        if len(player.hand) == 0:
            ...
            # print(player.hand)
            copy_card = Card.copy(player.hand[0])
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand) == 0:
            ...
            best_card = max(player.hand,
                            key=lambda card: card.power(opponent_card))
            index = player.hand.index(best_card)
