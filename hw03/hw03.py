...
def pingpong(n):
    ...
    True
    """
    # 此处可以按照文档说的，先写成一个while结构，
    # 然后看哪些参数是需要被传入和更新的，这一部分便用递归的天生结构嵌套作为参数
    """ 
    i=1,res=1,dir=1
    while i != n:
        if cond :
        ...
        res += 1*dir
    """
    def helper(index, result, dirction):
        if index == n:
            return result
        if num_eights(index) > 0 or index % 8 == 0:
            return helper(index+1, result-1*dirction, -dirction)
        else:
            return helper(index+1, result+1*dirction, dirction)
...
# 意思就是在有限长度的代码内多次调用函数实现最大的值？好像和midterm-exam有关系，所以我就没准备写了
