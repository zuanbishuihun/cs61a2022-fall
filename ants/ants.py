...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # 新加入的是普通蚂蚁，原地址是容器蚂蚁，然后可以容纳
            if self.is_container == False and place.ant.is_container and place.ant.can_contain(self):
                place.ant.store_ant(self)
            # 新加入的是容器蚂蚁，原地址是普通蚂蚁，新加入的容器可以容纳
            elif self.is_container == True and not place.ant.is_container and self.can_contain(place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur_range = 0
        while True:
            if cur_place.is_hive == True:
                return None
            if self.lower_bound <= cur_range <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if remain <= 0:
            ...
            for bee in bees:
                # 注意这个reduce_health方法在ant,bee都进行重写了
                # 关于place的ant,bee[]的处理也都包含了，不能重复处理了，已经remove的还怎么找得到
...
class WallAnt(Ant):
    """A WallAnt does nothing each turn but with a large health value."""
    name = 'Wall'
    food_cost = 4
    implemented = True   # Change to True to view in the GUI
...
class HungryAnt(Ant):
    """ Select a random bee at his same place.
        Consume the bee, causing damage.
        Spend 3 turns chewing.
        If there are no available bees to consume, take no action."""
    name = 'Hungry'
    food_cost = 4
    implemented = True   # Change to True to view in the GUI
    chewing_turns = 3  # 咀嚼周期

    def __init__(self, health=1):
        """Create an Ant with a HEALTH quantity."""
        super().__init__(health)
        self.turns_to_chew = 0  # 还有几个回合可以吞

    def action(self, gamestate):
        if self.turns_to_chew > 0:  # 在咀嚼中
            self.turns_to_chew -= 1
        elif self.turns_to_chew == 0:  # 可以吞了
            if len(self.place.bees) != 0:  # 当前位置有bee
                choose_bee = random_bee(self.place.bees)
                choose_bee.reduce_health(choose_bee.health)
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection and deals 1 damage to all bees in its place each turn"""
...
class ScubaThrower(ThrowerAnt):
    """A subclass of ThrowerAnt that is more costly and waterproof"""
    name = 'Scuba'
    food_cost = 6
    # OVERRIDE CLASS ATTRIBUTES HERE
    is_waterproof = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        super().action(gamestate)  # 先实现自己的攻击能力
        cur_place = self.place
        cur_place = cur_place.exit
        while cur_place is not None:
            if cur_place.ant is not None:  # 地上有蚂蚁
                if cur_place.ant.is_container and cur_place.ant.ant_contained is not None:  # 有容器蚂蚁 并且容器里面有蚂蚁
                    cur_place.ant.double()
                    cur_place.ant.ant_contained.double()
                else:  # 就一只蚂蚁 可能是容器蚂蚁，也有可能是普通蚂蚁
                    cur_place.ant.double()
            cur_place = cur_place.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    # problem EC 注意这个题目全部代码只能在本class中修改
    # 所以我就觉得很离谱，我看往年这个地方其实是对Bee的属性进行增加is_slow,slow_turns的
    # 这个地方直接对SlowThrower设置我觉得就很离谱，不应该是对投中的bee吗？
    # 但是很尴尬的是我没想出反面的例子。。。哈哈哈，只能说比较抽象
    name = 'Slow'
    food_cost = 6
    # BEGIN Problem EC
    implemented = True   # Change to True to view in the GUI
    slow_turning = 5  # 剩余减速回合
    # END Problem EC
    ...
    def throw_at(self, target):
        ...
        def target_action(gamestate):
            self.slow_turning -= 1
            # 只有偶数和没有减速回合了才会执行正常的Bee.action
            if gamestate.time % 2 == 0 or self.slow_turning < 0:
                Bee.action(target, gamestate)

        if target is not None:  # 扔到一个蚂蚁，对其创建一个临时的执行函数
            self.slow_turning = 5
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        cur_place = self.place
        cur_range = 0
        while cur_place.is_hive is False:
            # 记得抛开首位置的自己
            if cur_place.ant is not None and isinstance(cur_place.ant, LaserAnt) == False:
                insects_and_distances[cur_place.ant] = cur_range
            elif len(cur_place.bees) > 0:  # 有bee加bee
                bees = cur_place.bees[:]
                for bee in bees:
                    insects_and_distances[bee] = cur_range
            cur_place = cur_place.entrance
            cur_range += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        cur_damage = LaserAnt.damage - self.insects_shot * 0.0625 - distance * 0.25
        if cur_damage <= 0:
            return 0
        else:
            return cur_damage
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        # BEGIN Problem 12
        self.has_queen = False  # 添加是否已经创建了女皇的标志
        # END Problem 12
