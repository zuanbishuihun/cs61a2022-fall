...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    city_a_name = get_name(city_a)
    city_b_name = get_name(city_b)
    city_obj = make_city('obj', lat, lon)
    if distance(city_obj, city_a) < distance(city_obj, city_b):
        return city_a_name
    else:
        return city_b_name
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for brach in branches(t):
            if berry_finder(brach) == True:
                return True
        return False
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        # 如果长度不一样，那么是把长的那个branch后面的东西都加进去
