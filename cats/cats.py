...
def about(topic):
    ...
    def is_find_topic(para):
        para = lower(para)
        para = remove_punctuation(para)
        para_word_list = split(para)
        is_in = False
        for word in para_word_list:
            # print(word)
            if word in topic:
                is_in = True
                break
...
def accuracy(typed, source):
    ...
    if (t_len != 0 and s_len == 0) or (t_len == 0 and s_len != 0):
        return 0.0
    elif t_len == 0 and s_len == 0:
        return 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    # 这个地方题面要求了使用带有key=参数的min or max函数
    # 因此我创建了一个helper函数让其只接受一个参数，才能作用于每一个word_list中的word
    # 因此相当于我要事先搞定其他默认参数的绑定，最后得到只暴露一个word参数的函数helper
    def helper(word):
        return diff_function(typed_word, word, limit)

    if typed_word in word_list:
        return typed_word
    most_close_word = min(word_list, key=helper)
    # print(most_close_word)
    if diff_function(typed_word, most_close_word, limit) <= limit:
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    if limit < 0:
        return limit + 1  # 题目要求如果改变数超过了limit,那么最终返回任何大于limit的数字都行，其实也是在暗示用递归了
...
def minimum_mewtations(start, goal, limit):
    ...
    # 题目已经搭建了一些hint的框架了，和上一题目相比，编辑距离包含了三种操作：增加1个字符，删除1个字符，原地改变1个字符
    # 上一题只是原地改变1个字符的情况
...
def final_diff(typed, source, limit):
    ...
    # 简单改变FINAL_DIFF_LIMIT就可以得到很快的速度了，原始是6,我改成了3
    # 当然还有很多其他的改变方式，比如如果同时出现两个相同的字符，有可能是自己输错的
    if limit < 0:
        return limit + 1
...
FINAL_DIFF_LIMIT = 3  # REPLACE THIS WITH YOUR LIMIT
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(k):
        if typed[i] == prompt[i]:
            ...
            break
    right_ratio = right_cnt / p_len
    upload({'id': user_id, 'progress': right_ratio})
    return right_ratio
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    # new_p = []
    # for slice in times_per_player:
    #     new_p.append([slice[i]-slice[i-1] for i in range(1, len(slice))])
    # return match(words, new_p)
    # 直接一行解决战斗
    return match(words, [[p[i]-p[i-1] for i in range(1, len(p))] for p in times_per_player])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # 注意那个使用提供的抽象函数，比如time和get_word
    # 最关键的一点我觉得就是在fastest初始化的时候，直接用_占位有多少个人
    # 开始没有想到这样初始化，就会特别麻烦，因为不知道当前[]保存到什么index了
    fastest = [[] for _ in player_indices]  # 有多少个人就有多少个list ！！！
    for word_index in word_indices:  # 对于每个单词，找到谁打得最快，player_index
        fastest_time = float('inf')
        fastest_player_index = -1

        # 找到某个单词打字最快的人的index
        for player_index in player_indices:
            cur_time = time(match, player_index, word_index)  # 某个人打某个单词的时间
            if cur_time < fastest_time:
                fastest_time = cur_time
                fastest_player_index = player_index

        fastest[fastest_player_index].append(get_word(match, word_index))
