...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 1
        value = self.bindings.get(symbol)  # 先试图获取该Fram下的值
        if value is None and self.parent is None:  # 该Fram下没有且已经没有父亲Fram，报错
            # END PROBLEM 1
            raise SchemeError('unknown identifier: {0}'.format(symbol))
        elif value is None and self.parent is not None:  # 该Fram无，但是还可以继续向上查询
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 8
        child_fram = Frame(self)
        ptr_f = formals
        ptr_v = vals
        while ptr_f is not nil and ptr_v is not nil:
            child_fram.define(ptr_f.first, ptr_v.first)
            ptr_f = ptr_f.rest
            ptr_v = ptr_v.rest
        return child_fram
        # END PROBLEM 8
